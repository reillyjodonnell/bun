...
fn wasStepRequested(step_name: []const u8) bool {
    var args = std.process.argsWithAllocator(std.heap.page_allocator) catch return false;
    defer args.deinit();

    while (args.next()) |arg| {
        if (std.mem.eql(u8, arg, step_name)) {
            return true;
        }
    }
